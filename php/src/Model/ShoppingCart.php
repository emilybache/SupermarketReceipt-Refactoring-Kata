...
class ShoppingCart
{
     ...
     * @param SupermarketCatalog $catalog
     */
    public function handleOffers(Receipt $receipt, Map $offers, SupermarketCatalog $catalog): void
    {
        ...
        foreach ($this->productQuantities as $p => $quantity) {
            ...
            if ($offers->hasKey($p)) {
                ...
                if ($offer->getOfferType()->equals(SpecialOfferType::tenPercentDiscount())) $discount = new Discount($p, "{$offer->getArgument()}% off", $quantity * $unitPrice * $offer->getArgument() / 100.0);
